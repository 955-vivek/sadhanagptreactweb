...
const mapDBRulesToDraftScheme = (rules) => {
  ...
  rules.forEach(rule => {
    ...
    if (!scheme[key]) {
      console.log('Activity:', rule.activity_name, 'Frequency:', rule.frequency);
      scheme[key] = {
        id: rule.master_activity_id,
        title: (rule.activity_name || 'Unknown Activity') + (rule.activity_description && rule.activity_description.trim().toLowerCase() === 'weekly' ? ' (weekly)' : ''),
